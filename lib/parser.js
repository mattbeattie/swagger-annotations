...
var FUNCTION_NAME = /function\s+(\w+)\s*\(/g;
var FUNCTION_NAME_REF = /\w+(?:\.\w+)+/g;
var GRAMMAR = [
  'swaggerTag',
  'swaggerParameter',
  'swaggerPath',
  ...
  while (!ss.eos()) {
    ...
    if (tmpAnnotation.type === 'swaggerPath') {
      ...
      if ((match = routerDef.match(FUNCTION_NAME))) {
        var matches = [];
        while ( ( match = FUNCTION_NAME.exec(routerDef) ) && matches.push( match[1] ) ) {};
        match = matches[matches.length - 1];
        tmpAnnotation.operationId = match;
      }
      // Check if a function reference was used for the handler
      else if ((match = routerDef.match(FUNCTION_NAME_REF))) {
        match = match[match.length - 1];
        tmpAnnotation.operationId = match.split('.').pop();
      }
    }
